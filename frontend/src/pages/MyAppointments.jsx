...
const MyAppointments = () => {
  ...
  useEffect(() => {
    if (token && searchParams.get('success') && searchParams.get('appointmentId')) {
        const appointmentId = searchParams.get('appointmentId')
        const success = searchParams.get('success')

        const verifyPayment = async () => {
            try {
                const { data } = await axios.post(backendUrl + '/api/user/verify-stripe', { appointmentId, success }, { headers: { token } })
                if (data.success) {
                    toast.success(data.message)
                    getUserAppointments()
          ...
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {!item.cancelled && item.payment && !item.isCompleted && (
                <button className='text-stone-500 sm:min-w-48 py-2 border rounded bg-indigo-50'>Paid</button>
              )}
